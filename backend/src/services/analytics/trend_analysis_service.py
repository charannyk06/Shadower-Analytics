...
class TrendAnalysisService:
    ...
    def _generate_insights(
        ...
        if overview.get('trend'):
            insights.append({
                ...
                "recommendation": self._get_trend_recommendation(overview['trend'], overview.get('changePercentage', 0))
            })
        ...
        data = [
            {
                ...
                "movingAverage": float(row['moving_average']) if not pd.isna(row['moving_average']) else float(row['value']),
    ...
    async def _cache_analysis(
        ...
        try:
            ...
            query = """
                ...
                VALUES
                    (:workspace_id, :metric, :timeframe, :analysis_data, NOW() + INTERVAL ':hours hours')
    ...
    def _get_fallback_result(self, step_index: int) -> Dict[str, Any]:
        ...
        fallbacks = [
            ...
            {"seasonality": self._empty_seasonality(), "growth": self._empty_growth(), "cycles": []},
